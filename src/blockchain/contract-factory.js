  ...
  create({ name, abi }) {
    ...
    function _createInstance({ logger }) {
      return Object.create(
        _.assignIn({}, ContractFactory.objInterface, {
          contractReady: false,
          contractName: name,
    ...
    function _connect() {
      if (!_utils) {
        throw new Error(
          `Contract Instance for "${name}" has not been prepared!`,
        )
      }
    ...
    callContractFn(contractMethod, args) {
      if (!this.contractReady) {
        return Promise.reject(
          `Web3 Provider not ready (calling "${this.contractName}->${contractMethod}")`,
        )
      }
